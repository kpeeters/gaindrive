...
struct ServerListView: View {
	...
	@State private var editing: ServerEditorView.Target?
	@State private var confirmingRemoval: ServerConfig?
	@State private var statuses: [ServerId: ConnectionTest.Outcome] = [:]
	...
	var body: some View {
		...
		.sheet(item: $editing) { target in
			ServerEditorView(target: target)
		}
		.confirmationDialog(
	...
	private func refreshStatuses() async {
		...
		for config in registry.servers where config.isEnabled {
			guard let url = config.baseURL else {
				statuses[config.id] = .failed("The address is not usable.")
				continue
			}
			guard let password = registry.password(for: config.id) else {
				...
				statuses[config.id] = .failed("No saved password. Open the server to enter it again.")
				continue
			}
			targets.append((config.id, url, config.username, password))
		}

		let results = await withTaskGroup(of: (ServerId, ConnectionTest.Outcome).self) { group in
			for target in targets {
				group.addTask {
					(
						target.id,
						await ConnectionTest.run(
							baseURL: target.url,
			...
			var collected: [(ServerId, ConnectionTest.Outcome)] = []
...
private struct ServerRow: View {
	...
	let status: ConnectionTest.Outcome?
	...
	private var dotDescription: String {
		...
		switch status {
		case .ok, .okWithoutRoles: return "Reachable"
		case .wrongCredentials: return "Wrong credentials"
