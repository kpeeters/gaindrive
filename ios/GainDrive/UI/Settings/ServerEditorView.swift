...
struct ServerEditorView: View {
	...
	@State private var testResult: ConnectionTest.Outcome?
	...
	private func runTest() async {
		guard let baseURL = URL(string: normalisedURL) else {
			testResult = .failed("The address is not usable.")
			return
		}
		isTesting = true
		testResult = nil
		testResult = await ConnectionTest.run(
...
private struct TestResultRow: View {
	let outcome: ConnectionTest.Outcome
	...
	private var title: String {
		switch outcome {
		case .ok(let user): "Connected as \(user.username)"
		case .okWithoutRoles: "Connected"
