...
fun RecentsScreen(
	...
	Scaffold(
		...
		RefreshableLoadBox(
			...
			if (songs.isEmpty()) {
				EmptyMessage("Nothing played yet on this server.")
			...
			LazyColumn(modifier = Modifier.fillMaxSize()) {
				itemsIndexed(
					items = songs,
					// A track played twice appears twice, so the id alone is not
					// unique here.
					...
					SongRow(
						...
						// Both, deliberately: playback starts here, and the
						// album opens so the rest of the record is one tap
						// away. This is what the web client does from a
						// listing.
						onClick = {
							viewModel.playInAlbumContext(row.song)
							row.song.albumRef?.let { onOpenAlbum(it, row.song.albumTitle) }
						},
