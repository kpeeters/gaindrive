...
function playerPlay(offset = 0, forceMp3 = false) {
   ...
   if (offset > 0 && !fmt) {
      // Direct serve supports byte-Range — let the browser seek natively.
...
player.audio.addEventListener('timeupdate', () => {
   ...
   const cur  = player.audio.currentTime + (player.localOffset || 0);
   ...
   const song = player.queue[player.index];
   const dur  = song?.duration || player.audio.duration || 0;
...
function setupPlayer() {
   ...
   document.getElementById('player-prev').addEventListener('click', () => {
      const absCur = player.audio.currentTime + (player.localOffset || 0);
      if (absCur > 3) {
