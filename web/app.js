...
function onCastStatus(s) {
   ...
   // The receiver owns the spinner for the same reason it owns the glyph:
   // what the local element is doing is not what the listener can hear.
   playerBuffering(s.playerState === 'BUFFERING');
   }
...
const player = {
   ...
   // The track has been asked for but is not audible yet.  Set from the click
...
// metadata several times a track.  playerUpdateUI() re-applies it from the
// flag, which is what keeps a re-rendered track list correct.
function playerBuffering(on) {
   player.buffering = on;
   document.querySelector('.track-row.playing')
      ?.classList.toggle('buffering', on);
}
...
// lower it when sound actually starts, and raise it again for a mid-stream
// stall.  `playing` rather than `canplay` is what clears it, because a
// chunked transcode can be decodable before it is audible; `canplay` is here
// only for the paused-seek path, which re-fetches and never reaches `playing`.
//
// Guarded like the two above, and for the same reason: while casting, this
// element is paused and played by castSyncTick() to follow the receiver, so
// its stalls are the local picture's rather than the listener's.  onCastStatus
// owns the flag for a cast session.
...
for (const ev of ['playing', 'canplay', 'error']) {
   el.addEventListener(ev, () => {
      if (castDeviceId !== null) return;
      playerBuffering(false);
      });
